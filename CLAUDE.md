...
- `internal/services/` — all Go services exposed to the frontend. One file per service.
  ...
  inheritance and provenance, and is the only thing that writes one.
...
- `frontend/` — Vite + React + TS. Path aliases `@/*` → `frontend/src/*` and
  ...
  - `state/` — React context providers, one concern each (`settings-context`,
    `collection-context`, `tabs-context`, `documents-context`), each exporting
    a `useXxx` hook that throws outside its provider. Providers are composed in
    `routes/__root.tsx`; `documents-context` sits inside `tabs-context`,
    because a draft is what makes a tab dirty.
